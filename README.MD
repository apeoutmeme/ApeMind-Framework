...
# STARTING THE UI AND SERVER TOGETHER: 
First, start the proxy server (which handles the Jupiter API requests):
`node proxyServer.js`

Then in a separate terminal, navigate to the solana-ui directory and start the frontend:
```
cd solana-ui
pnpm dev
```
## another way to start the UI and server together: 
To make this process easier, you could add a new script to your root package.json to run both simultaneously. Add this to your package.json scripts:

To make this process easier, you could add a new script to your root package.json to run both simultaneously. Add this to your package.json scripts:
First install concurrently:
`pnpm add -g concurrently`
Then you can start both with a single command:
`pnpm start`
The frontend will be available at http://localhost:5173 (Vite's default port) and the proxy server will run on http://localhost:3001 as configured in the proxyServer.js file.

Remember to ensure you have all dependencies installed in both the root directory and the solana-ui directory:
`pnpm install`
`cd solana-ui && pnpm install`